...
describe('norma', function () {
  ...
  it('no-match', function () {
    try {
      ...
      assert.equal('invalid_arguments', e.code);assert.ok(e.message.indexOf('invalid'));
    }
  ...
  it('no-args', function () {
    ...
    try {
      var compiled = norma.compile('s');compiled();assert.fail();
    } catch (e) {
      assert.equal('init', e.code);
      assert.ok(~e.message.indexOf('no arguments variable'));
    }
  ...
  it('error-msg', function () {
    try {
      norma('s', [1, { a: 1 }, [10, 20]]);assert.fail();
    } catch (e) {
      assert.equal('invalid_arguments', e.code);
...
//# sourceMappingURL=norma.test.js.map