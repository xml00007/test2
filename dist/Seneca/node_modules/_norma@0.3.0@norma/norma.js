...
function processargs(specdef, options, rawargs) {
  ...
  if (!outslots) {
    if ('throw' == options.onfail) {
      throw error('invalid_arguments', 'invalid arguments; expected: "' + specdef.spec + '", was: [' + argdesc + ']; values: ' + descargs(args, options), { args: args, specdef: specdef, options: options });
    } else return null;
  }
...
function handle(specdef, options, rawargs) {
  ...
  if (null == rawargs) {
    throw error('init', 'no arguments variable; expected norma( "...", arguments ), ' + 'or <compiled>( arguments )', { arguments: arguments });
...
module.exports.compile = function (spec) {
  ...
  var out = function out(options, rawargs) {
    return handle(specdef, options, rawargs);
  };
